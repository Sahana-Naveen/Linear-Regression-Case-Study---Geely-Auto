...
#*** Data set : 370k used carprice data base from ebay with 19 variables.

#*** Agenda:

# Creating Price prediction model using regression analysis.
# Find if the car will be sold or not using classification analysis

#---------------------------------------------------------------------

#Set the directory Path
setwd("C:/Data_Science/Geely Auto - Linear regression")
...
Input <- read.csv("CarPrice_Assignment.csv",stringsAsFactors = F)
str(Input)

#*********************************************************************************************

# Check for blank and Missing Values
length(which(is.na(Input))) # No NA values found
sapply(Input, function(x) length(which(x == ""))) # No blank values
sum(duplicated(Input)) #No duplicates


# Variable "CarName" is split into "CompanyName" and "ModelName"

carprice <- separate(Input, CarName, into = c("Company","ModelName"), 
...
#changing variables to factor values
...
summary(carprice$Company) # incorrect spelling of the same company name found
...
#r squared from test dataset is 0.877 and initial model/traning data is 0.9511. The difference is approx 7%, which is
...
#Depending on these variables/parameters the Geely Auto manufacturers can develop appropriate car models with reasonable pricing.
